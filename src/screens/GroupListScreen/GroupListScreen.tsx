  ...
  const renderMemberItem = ({item, index}: ListRenderItemInfo<TAccount>) => {
    return (
      <View key={index} style={styles.memberContainer}>
        <AppImage
          source={{uri: 'https://static.vecteezy.com/system/resources/previews/021/770/056/non_2x/avatar-of-a-student-character-free-vector.jpg'}}
          style={styles.avatar}
        />
        <View style={{flex: 1}}>
    ...
    return (
      <View key={index} style={styles.memberContainer}>
        ...
        <View style={{flex: 1}}>
          ...
          <Text style={[styles.desc, item.status ? {color: COLORS.green} : {color: COLORS.red}]}>{item.status ? 'Đang hoạt động' : 'Tạm dừng'}</Text>
        </View>
        <TouchableOpacity>
          <Feather name="x" size={s(20)} color={COLORS.red} />
        </TouchableOpacity>
